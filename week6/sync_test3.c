...
sem_t bin_sem, bin_sem1, bin_sem2, bin_sem3;
...
int main(){
	...
	res3 = sem_init(&bin_sem3, 0, 0);
	// 세마포어 bin_sem1 생성 후 1로 할당
	// 세마포어 bin_sem2 생성 후 0으로 할당
	// 세마포어 bin_sem3 생성 후 0으로 할당
	...
	res3 = pthread_create(&thread3, NULL, thread_function3, NULL);
	// 쓰레드 각각 생성
	...
	res3 = pthread_join(thread3, &thread_result);
	// 쓰레드 종료
...
void *thread_function2(void *arg){
	...
	for(i=0;i<LOOP_COUNTER;++i){
		...
		sem_post(&bin_sem3); // bin_sem3 : 0으로 할당 -> 쓰레드3 작동
	}
...
void *thread_function3(void *arg){
	int i=0;
	for(i=0;i<LOOP_COUNTER;++i){
		sem_wait(&bin_sem3); // bin_sem3 : 0이면 아래코드 실행
		sem_wait(&bin_sem); // bin_sem : 0이면 아래코드 실행
		counter += 3;
		sem_post(&bin_sem); // bin_sem : 0으로 할당
		printf("Thread3_counter: %d\n", counter);
		sem_post(&bin_sem1); // bin_sem1 : 0으로 할당 -> 쓰레드1 작동
	}
	pthread_exit("Thread3 EXIT");
}
